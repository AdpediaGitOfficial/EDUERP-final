...
describe("validation (400)", () => {
  it("rejects missing class", async () => {
    const r = await admit("admin", {
      firstName: "X",
      parentMode: "new",
      parentLoginEmail: `a${S}@p.test`,
      father: { name: "F" },
    });
...
describe("RBAC (403)", () => {
  it("teacher cannot admit", async () => {
    const r = await admit("teacher", {
      classId,
      firstName: "X",
      parentMode: "new",
      parentLoginEmail: `t${S}@p.test`,
      father: { name: "F" },
    });
    expect(r.status).toBe(403);
  });
  it("parent cannot admit", async () => {
    const r = await admit("parent", {
      classId,
      firstName: "X",
      parentMode: "new",
      parentLoginEmail: `p${S}@p.test`,
      father: { name: "F" },
    });
    expect(r.status).toBe(403);
  });
  ...
  it("full record persists details, medical, custom fields and category", async () => {
    ...
    const r = track(
      await admit("admin", {
        ...
        bankAccount: "999",
        bankIfsc: "HDFC0001",
        currentAddress: "Addr A",
        permanentAddress: "Addr B",
        parentMode: "new",
        father: { name: "F" },
        parentLoginEmail: email,
    ...
    expect(r.status).toBe(201);
    const det = await prisma.student_details.findUnique({
      where: { student_id: r.body.studentId },
    });
    ...
    const med = await prisma.student_medical.findUnique({
      where: { student_id: r.body.studentId },
    });
  ...
  it("father primary + mother block → two guardians (one login-less)", async () => {
    ...
    const r = track(
      await admit("admin", {
        ...
        primaryGuardian: "father",
        father: { name: `Dad${S}` },
        mother: { name: `EdgeMom ${S}` },
        parentLoginEmail: email,
      }),
      email,
    );
    expect(r.status).toBe(201);
    const guardians = await prisma.parent_student.count({
      where: { student_id: r.body.studentId },
    });
...
describe("duplicate prevention (409)", () => {
  ...
  it("duplicate parent phone (matches David)", async () => {
    const r = await admit("admin", {
      ...
      firstName: `DP${S}`,
      parentMode: "new",
      father: { name: "F", phone: "+91 90000 10001" },
      parentLoginEmail: `dp${S}@p.test`,
    });
    expect(r.status).toBe(409);
  });
  it("duplicate national ID (matches David)", async () => {
    const r = await admit("admin", {
      ...
      firstName: `DN${S}`,
      parentMode: "new",
      father: { name: "F", aadhaar: "NID-DAVID-001" },
      parentLoginEmail: `dn${S}@p.test`,
    });
    expect(r.status).toBe(409);
  });
  it("duplicate manual admission number", async () => {
    const existing = (await prisma.students.findFirst({
      where: { admission_no: { not: null } },
      select: { admission_no: true },
    }))!.admission_no!;
    const r = await admit("admin", {
      classId,
      firstName: `DA${S}`,
      admissionNo: existing,
      parentMode: "new",
      father: { name: "F" },
      parentLoginEmail: `da${S}@p.test`,
    });
    expect(r.status).toBe(409);
  });
  it("duplicate roll in the same class", async () => {
    const email1 = `r1${S}@p.test`,
      email2 = `r2${S}@p.test`;
    track(
      await admit("admin", {
        classId: classId2,
